...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        img_emb = self.visual_projection(features)
        cap_emb = self.embedding(captions)
        cap_emb = self.positional_encoding(cap_emb)
        tgt_mask = torch.tril(
            torch.ones(T, T, device=cap_emb.device, dtype=cap_emb.dtype)
        )
        img_emb = img_emb.unsqueeze(1)
        assert not torch.isnan(img_emb).any(), "NaN values in image embedding"
        out = self.transformer(cap_emb, img_emb, tgt_mask)
