...
def discriminator_loss(logits_real, logits_fake):
    ...
    r = bce_loss(logits_real, torch.ones(logits_real.shape[0]).type(dtype))
    f = bce_loss(logits_fake, torch.zeros(logits_fake.shape[0]).type(dtype))
    loss = r + f
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    r = 0.5 * torch.mean((scores_real - 1) ** 2)
    f = 0.5 * torch.mean((scores_fake) ** 2)
    loss = r + f
...
def build_dc_classifier(batch_size):
    ...
    return nn.Sequential(
        ...
        nn.Linear(
            64 * 4 * 4, 64 * 4 * 4
        ),  # Assuming the input dimensions and convolutions reduce to 4x4 at this layer
        nn.LeakyReLU(negative_slope=0.01),
        nn.Linear(64 * 4 * 4, 1),
    )
