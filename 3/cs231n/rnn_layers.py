...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h[:, 0, :] = h0
    for t in range(T):
        h[:, t + 1, :], cache_step = rnn_step_forward(x[:, t, :], h[:, t, :], Wx, Wh, b)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, prev_h, prev_c, Wx, Wh, b, next_h, next_c, ai, af, ao, ag, i, o, f, g)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    (x, prev_h, prev_c, Wx, Wh, b, next_h, next_c, ai, af, ao, ag, i, o, f, g) = cache
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dx[:, t, :], dprev_h, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(
            dh[:, t, :] + dprev_h, dc[:, t, :] + dprev_c, cache[t]
        )
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
