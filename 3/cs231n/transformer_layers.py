...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        position = torch.arange(0, pe.shape[1], dtype=torch.float32).unsqueeze(1)
        div_term = torch.exp(
            torch.arange(0, pe.shape[2], 2).float() * (-math.log(10000.0) / pe.shape[2])
        )
    ...
    def forward(self, x):
        ...
        output = x + self.pe[: x.size(0), : x.size(1)]
